...
public final class ColumnsInterval implements Comparable<ColumnsInterval> {
  ...
  private boolean isValid(String prefix) {
    if (prefix==null || prefix.contains("..") || prefix.length()!=0 && prefix.charAt(0)=='.') {
      return false;
    }
    return true;
  }
  ...
  public ColumnsInterval(String column) {
    if (StringUtils.isContainRegex(column) || !isValid(column) || StringUtils.isContainSpecialChar(column)) {
