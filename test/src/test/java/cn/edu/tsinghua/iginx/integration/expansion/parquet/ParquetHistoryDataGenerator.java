...
public class ParquetHistoryDataGenerator extends BaseHistoryDataGenerator {
  ...
  public void writeHistoryData(
    ...
    LOGGER.info("start writing data, dir:{}, filename:{}.", new File(dir).getAbsolutePath(), filename);
  ...
  public void clearHistoryDataForGivenPort(int port) {
    ...
    if (file.exists() && file.isFile()) {
      ...
      LOGGER.warn("delete {}/{} error: does not exist or is not a file.", dir, file.getAbsoluteFile());
    }

    // delete the normal IT data
    parquetPath = Paths.get(IT_DATA_DIR);

    try {
      Files.walkFileTree(parquetPath, new DeleteFileVisitor());
    } catch (NoSuchFileException e) {
      LOGGER.warn("no such file or directory: {}", new File(parquetPath.toString()).getAbsoluteFile());
    } catch (IOException e) {
